...
async function generateLeaderboard(results, sprintDurationMs) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    const avatarBuffer = user.avatar_url
      ? await fetchAvatar(user.avatar_url)
      : null;
    if (avatarBuffer) {
    ...
    const books = Array.isArray(user.books)
      ? user.books.filter((b) => b.title)
      : [];
    const bookTitles = books.map((b) => b.title).join(", ") || "—";
    const activeDuration =
      user.leave_time && user.join_time
        ? new Date(user.leave_time) -
          new Date(user.join_time) -
          Number(user.total_pause_ms)
