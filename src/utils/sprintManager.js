...
async function sendAdminPanel(channel, sprint = null) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId("sprint_schedule")
      .setLabel("📅 Schedule Sprint")
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId("sprint_start")
      .setLabel("▶️ Start Now")
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId("sprint_end")
      .setLabel("⏹️ End Sprint")
...
async function sendUserPanel(channel, sprint) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId("sprint_join")
      .setLabel("📖 Join Sprint")
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId("sprint_edit_leave")
      .setLabel("✏️ Edit / Leave")
      .setStyle(ButtonStyle.Secondary),
    new ButtonBuilder()
      .setCustomId("sprint_pause")
      .setLabel("⏸️ Pause")
      .setStyle(ButtonStyle.Primary),
  );
    ...
    .addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          ...
          .setLabel(
            `Current page/minute (was: ${currentBook?.start_page ?? 0})`,
          )
          .setStyle(TextInputStyle.Short)
          .setRequired(true),
      ),
...
async function handleScheduleModal(interaction) {
  ...
  const startTime = interaction.fields
    .getTextInputValue("schedule_start")
    .trim();
  const endTime = interaction.fields.getTextInputValue("schedule_end").trim();
  const repeat =
    interaction.fields
      .getTextInputValue("schedule_repeat")
      .trim()
      .toLowerCase() || "none";
  ...
  if (isNaN(scheduledStart) || isNaN(scheduledEnd)) {
    return interaction.reply({
      content: "❌ Invalid date or time format. Use DD.MM.YYYY and HH:MM.",
      ephemeral: true,
    });
  }
  if (scheduledStart < new Date()) {
    return interaction.reply({
      content: "❌ Start time is in the past!",
      ephemeral: true,
    });
  }
  ...
  if (existing) {
    return interaction.reply({
      content: "❌ There is already an active or scheduled sprint!",
      ephemeral: true,
    });
  }
  ...
  await interaction.channel.send({
    content: `📅 **A Reading Sprint has been scheduled!**\n🗓️ **${date}** from **${startTime}** to **${endTime}**\n🔁 Repeat: **${repeat}`,
  });

  await interaction.reply({
    content: `✅ Sprint scheduled!\n📅 **${date}** from **${startTime}** to **${endTime}**\n🔁 Repeat: **${repeat}**`,
    ephemeral: true,
  });
  ...
  setTimeout(
    () => startSprint(interaction.client, interaction.channelId),
    msUntilStart,
  );
  setTimeout(
    () => endSprint(interaction.client, interaction.channelId),
    msUntilEnd,
  );
}
...
async function handleJoinModal(interaction) {
  ...
  if (!sprint || sprint.status !== "active") {
    return interaction.reply({
      content: "❌ No active sprint right now.",
      ephemeral: true,
    });
  }
  ...
  if (existing) {
    return interaction.reply({
      content:
        "❌ You are already in this sprint! Use **Edit / Leave** to update.",
      ephemeral: true,
    });
  }
  ...
  const format =
    interaction.fields.getTextInputValue("join_format").trim().toLowerCase() ||
    "book";
  const startPage =
    parseInt(interaction.fields.getTextInputValue("join_start_page").trim()) ||
    0;
  const totalPages =
    parseInt(interaction.fields.getTextInputValue("join_total_pages").trim()) ||
    0;
...
async function handleEditLeaveModal(interaction) {
  ...
  if (!participant) {
    return interaction.reply({
      content: "❌ You are not in this sprint.",
      ephemeral: true,
    });
  }
  ...
  if (!currentBook) {
    return interaction.reply({
      content: "❌ No book found for your entry.",
      ephemeral: true,
    });
  }

  const endPage =
    parseInt(interaction.fields.getTextInputValue("edit_end_page").trim()) ||
    currentBook.start_page;
  const goalReachedRaw = interaction.fields
    .getTextInputValue("edit_goal_reached")
    .trim()
    .toLowerCase();
  const goalReached = goalReachedRaw === "yes";
  const nextAction =
    interaction.fields
      .getTextInputValue("edit_next_action")
      .trim()
      .toLowerCase() || "stay";
  const nextBookTitle = interaction.fields
    .getTextInputValue("edit_next_book")
    .trim();
  const nextStartPage =
    parseInt(
      interaction.fields.getTextInputValue("edit_next_start_page").trim(),
    ) || 0;
  ...
  if (nextAction === "leave") {
    ...
    return interaction.reply({
      content: `👋 You left the sprint. Pages read: **${endPage - currentBook.start_page}**`,
      ephemeral: true,
    });
    ...
    return interaction.reply({
      content: `📖 Switched to **${nextBookTitle}**, starting on page **${nextStartPage}**!`,
      ephemeral: true,
    });
  } else {
    return interaction.reply({
      content: `✅ Progress saved! You're on page **${endPage}**.`,
      ephemeral: true,
    });
  }
}
...
async function handlePause(interaction) {
  ...
  if (participant.is_paused) {
    ...
    return interaction.reply({
      content: "▶️ Unpaused! Your timer is running again.",
      ephemeral: true,
    });
  } else {
    // Pause
    ...
    return interaction.reply({
      content: "⏸️ Paused! Your time is not counting until you unpause.",
      ephemeral: true,
    });
  }
}
...
async function startSprint(client, channelId) {
  ...
  await channel.send({
    content: "@here 📚 **The Reading Sprint has started! Join now!**",
  });
...
async function endSprint(client, channelId) {
  ...
  await pool.query(
    `UPDATE sprints SET status = 'ended', end_time = NOW() WHERE id = $1`,
    [sprint.id],
  );
  ...
  await channel.send({
    content:
      "@here ⏹️ **The sprint has ended!** You have **5 minutes** to update your final page count using the **Edit / Leave** button above.",
  });
...
async function postLeaderboard(client, channelId, sprint) {
  ...
  for (const r of results) {
    try {
      ...
      r.avatar_url = member.user.displayAvatarURL({
        extension: "png",
        size: 128,
      });
    } catch {
  ...
  const sprintDuration =
    sprint.end_time && sprint.start_time
  ...
  await channel.send({
    content: "@here 🏆 **Sprint Leaderboard!**",
    files: [attachment],
  });
  ...
  setTimeout(
    async () => {
      await pool.query(`UPDATE sprints SET status = 'idle' WHERE id = $1`, [
        sprint.id,
      ]);
    },
    60 * 60 * 1000,
